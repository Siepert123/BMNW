...
public class DudBlockEntity extends BlockEntity {
    boolean isExploding;
    int explosionCurrent;
    final int explosionCurrentMax = 32;

    public DudBlockEntity(BlockPos pPos, BlockState pState) {
        super(ModBlockEntities.DUD_BE.get(), pPos, pState);
    }
    ...
    private void serverLevelActions(Level pLevel, BlockPos pPos, BlockState pState, Random pRandom) {
        if (isExploding) {
            if (explosionCurrent > explosionCurrentMax) {
                ...
                BombUtils.setAreaAflame(pLevel, pPos.getX(), pPos.getY(), pPos.getZ(), 64, 16);
                isExploding = false;
